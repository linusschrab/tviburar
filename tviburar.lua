...
function init_params()
  params:add_group("midi & outputs", 24)
...
--[[     params:set_action("twin"..i.."crow_2", function (x)
    ...
    --[[ params:set_action("twin"..i.."JF", function(x)
      if params:get("twin".. util.wrap(i+1,1,4) .."JF") == 1 or params:get("twin".. util.wrap(i+2,1,4) .."JF") == 1 or params:get("twin".. util.wrap(i+3,1,4) .."JF") == 1 then
        if x == 2 then
          crow.ii.jf.mode(1)
        else
          crow.ii.jf.mode(0)
  ...
  params:set_action("root_note", function (x) 
    scale = music.generate_scale(x-1, params:get("scale"), 10) 
  end)
  for i=1,2 do
