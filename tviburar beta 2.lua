...
function init()
  ...
  lane_timers = {
    lane_1_timer = time_handler:new_pattern{
      action = function (x)
        lanes["lane_1"]['active_lfo'] = lanes["lane_1"]['active_lfo'] + 1
        lfo_res_step = 0
        if lanes["lane_1"]['active_lfo'] > 4 then lanes["lane_1"]['active_lfo'] = 1 end
    ...
    lane_2_timer = time_handler:new_pattern{
      action = function (x) 
        lanes["lane_2"]['active_lfo'] = lanes["lane_2"]['active_lfo'] + 1
        if lanes["lane_2"]['active_lfo'] > 4 then lanes["lane_2"]['active_lfo'] = 1 end
      end,
      division = 1/4
    }
  }
...
function init_params()
  ...
  for i=1,2 do
    params:add_option("lane_"..i.."_division","lane "..i.." division", {1/4, 1/3, 1/2, 1, 2, 3, 4}, 4)
    params:set_action("lane_"..i.."_division", function(x)
      lane_timers["lane_"..i.."_timer"]:set_division(params:get(params:get("lane_"..i.."_division")))
    end)
  end
  for i=1,2 do
    for j=1,4 do
...
function update_lfos()
    ...
    if lanes["lane_1"]['lfos']['lfo_'..lanes["lane_1"]['active_lfo']]['shape'] == 'square' then
      ...
      --end
      if lfo_res_step * 2 >= math.floor(lfo_resolution / params:get("lane_1_lfo_"..lanes["lane_1"]['active_lfo'].."_rate")) then
      --lane_1['lfos']['lfo_'..lane_1['active_lfo']]['rate']) then
        
        current_lfo_val = current_lfo_val * -1
        lfo_res_step = 0
        play(math.floor(current_lfo_val * params:get("lane_1_lfo_"..lanes["lane_1"]['active_lfo'].."_amp") * 12))
